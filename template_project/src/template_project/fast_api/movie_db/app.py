...
def retrieve_all_movies_details(
...
def add_new_movie(movie: schema.MovieAdd, db: Session = Depends(get_db)):
    ...
    if movie_id:
        raise HTTPException(
            status_code=400,
            detail=f"Movie id {movie.movie_id} already exist in database: {movie_id}",
        )
...
def update_movie_details(
    sl_id: int, update_param: schema.UpdateMovie, db: Session = Depends(get_db)
):
    details = crud.get_movie_by_id(db=db, sl_id=sl_id)
    if not details:
